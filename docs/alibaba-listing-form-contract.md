...
不能退化成自由文本。没有内联选项且没有 `asyncQueryRule` 的选择字段、未知字段、未知枚举
和未识别的复合字段默认交给客户，不能默认交给 AI。
...
| 客户填写 | 最终叶子类目、API 未识别字段、类目特殊声明、权利和合规确认、公司素材、视频选择、服务承诺、无法从 ERP 获取的字段 | 默认 `user_provided`，不得由 AI 猜测 |
