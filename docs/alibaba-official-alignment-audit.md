...
- 新增全店通用/每商品、AI 辅助/可信来源专属的四象限字段矩阵；
- 新增 `user_confirmed` 和 `account_default` 来源，店铺默认值只允许白名单字段；
- AI/图片提取字段即使漏标人工确认，也会因来源被阻止进入写操作；
- 新增来源门禁驱动的单品/批量草稿和正式发布接口，由后端构建 XML 后写入。
